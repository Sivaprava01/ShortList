...
import { HiOutlineCheckCircle, HiOutlinePlusCircle, HiOutlineTrash, HiOutlineLightBulb } from 'react-icons/hi2';
...
export default function ProfileBuilder() {
  ...
  const sectionClass = "bg-card rounded-2xl border border-custom p-6 space-y-4";
  const labelClass = "block text-sm font-medium text-primary mb-1.5";
  const inputClass = "w-full px-4 py-2.5 bg-primary border border-custom rounded-xl text-primary placeholder:text-secondary focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-sm";

  const { score: strengthScore, suggestions } = analyzeProfile(form);
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="bg-card rounded-2xl border border-custom p-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="font-semibold text-primary">Profile Strength</h2>
            <p className="text-xs text-secondary mt-0.5">Complete your profile to improve matching</p>
          </div>
          <span className={`text-2xl font-bold ${
            strengthScore >= 80 ? 'text-emerald-500' :
            strengthScore >= 50 ? 'text-amber-500' : 'text-red-500'
          }`}>{strengthScore}%</span>
        </div>
        <div className="w-full bg-primary rounded-full h-2.5 mb-4">
          <div
            className={`h-2.5 rounded-full transition-all duration-700 ${
              strengthScore >= 80 ? 'bg-emerald-500' :
              strengthScore >= 50 ? 'bg-amber-500' : 'bg-red-500'
            }`}
        ...
        {suggestions.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs font-semibold text-secondary uppercase tracking-wide flex items-center gap-1.5">
              <HiOutlineLightBulb className="w-3.5 h-3.5 text-amber-500" />
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between">
          ...
          <button onClick={addExperience} className="flex items-center gap-1 text-sm text-indigo-500 font-medium hover:underline">
            <HiOutlinePlusCircle className="w-4 h-4" /> Add Experience
          </button>
        </div>
        {form.experience.map((exp, i) => (
          <div key={i} className="p-4 bg-primary rounded-xl border border-custom space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-secondary">Experience {i + 1}</span>
        ...
        <div className="sm:w-48">
          ...
          <input type="number" min="0" className={`${inputClass} opacity-70`} value={form.experience.reduce((total, exp) => total + (exp.years || 0), 0) || form.experienceYears} onChange={e => setForm(f => ({ ...f, experienceYears: Number(e.target.value) }))} placeholder="Manual override" />
          <p className="text-xs text-secondary mt-1">Auto-calculated from experience entries above</p>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between">
          ...
          <button onClick={addEducation} className="flex items-center gap-1 text-sm text-indigo-500 font-medium hover:underline">
        ...
        {form.education.map((edu, i) => (
          <div key={i} className="p-4 bg-primary rounded-xl border border-custom space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-secondary">Education {i + 1}</span>
