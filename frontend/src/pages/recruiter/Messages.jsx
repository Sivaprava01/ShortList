        ...
        return (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gradient-to-br from-[var(--accent)] to-purple-600 rounded-xl flex items-center justify-center shrink-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                {chat.lastMessage && (
                  <p className="text-xs text-[var(--text-secondary)] truncate mt-1 opacity-60">{chat.lastMessage}</p>
                )}
              </div>
            </div>
          </button>
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="w-6 h-6 border-3 border-[var(--accent)] border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSend} className="p-4 border-t border-[var(--border)]">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-[var(--bg-primary)] border border-[var(--border)] rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-transparent text-[var(--text-primary)]"
          />
          <button
            type="submit"
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[var(--text-secondary)] mt-1">Chat with candidates about job opportunities.</p>
      </div>

      {chats.length === 0 ? (
        <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-16 text-center">
          <HiOutlineChatBubbleLeftRight className="w-16 h-16 text-[var(--text-secondary)] mx-auto mb-4 opacity-40" />
          <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">No conversations yet</h3>
          <p className="text-[var(--text-secondary)]">Start a conversation from the Applicants page to begin chatting with candidates.</p>
        </div>
      ) : (
        <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] overflow-hidden" style={{ height: 'calc(100vh - 220px)', minHeight: '500px' }}>
          <div className="flex h-full">
            <div className="w-80 border-r border-[var(--border)] flex flex-col shrink-0">
              <div className="p-4 border-b border-[var(--border)]">
                <h2 className="font-semibold text-[var(--text-primary)] text-sm">Conversations ({chats.length})</h2>
              </div>
